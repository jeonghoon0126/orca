...
export function getHeavySuiteLockPath(tempDir = os.tmpdir()) {
  return path.join(tempDir, HEAVY_SUITE_LOCK_BASENAME)
}
...
export function isProcessTreeAlive(
  ...
  if (platform === 'win32' || !isValidPid(pid)) {
    return false
  }
  try {
    killProcess(-pid, 0)
...
function assertRecoverableObservation(observation, { suite, isOwnerAlive, isChildTreeAlive }) {
  ...
  if (observation.kind === 'malformed') {
    if (!observation.regularFile) {
      throw new HeavySuiteBusyError(
        `[heavy-suite] Cannot start ${suite}: the shared lock is not a regular file. Inspect ${HEAVY_SUITE_LOCK_BASENAME} without deleting it recursively.`
      )
    }
  ...
  if (currentOwner.phase === 'spawning') {
    throw new HeavySuiteBusyError(
      `[heavy-suite] Cannot start ${suite}: ${currentOwner.suite} stopped while starting a child process. Verify that exact run before removing ${HEAVY_SUITE_LOCK_BASENAME}.`
    )
  }
  return true
}
...
function recoverObservedStaleLock({
  ...
  try {
    ...
    if (
      ...
      assertRecoverableObservation(current, { suite, isOwnerAlive, isChildTreeAlive })
    ) {
...
export function acquireHeavySuiteLock({
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    assertRecoverableObservation(observed, { suite, isOwnerAlive, isChildTreeAlive })
